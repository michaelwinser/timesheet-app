...
<div
	class="bg-white border rounded-lg p-3 {editable && !isLocked ? 'hover:shadow-sm cursor-pointer' : ''} {isLocked ? 'opacity-75' : ''}"
>
	{#if editing}
		<div class="space-y-2">
		...
		<div class="flex items-center justify-between" onclick={handleEdit}>
			<div class="flex items-center gap-2">
				...
				{#if entry.description}
					<span class="text-gray-500 text-sm truncate max-w-xs">{entry.description}</span>
				{/if}
			</div>
			<div class="flex items-center gap-2">
				{#if isLocked}
